...
TUNNEL_GIT_PATH=$SOURCE_ROOT/
BEAR_GIT_PATH=$SOURCE_ROOT/../bear/

# Finds the version of Tunnel
...
RESULT_DIRECTORY_NAME=tunnel-$TUNNEL_VERSION
...
cd $TUNNEL_GIT_PATH
git archive master --prefix=tunnel/ | tar -x -C $SOURCE_DIRECTORY
...
mv tunnel/build-scripts/wine-make.sh .

# Creates the final archive in the directory from which the script was started.
cd $WORKING_DIRECTORY/
tar cfz $INITIAL_DIRECTORY/tunnel_$TUNNEL_VERSION.tar.gz \
