...
ANDROID_TUNNEL_VERSION=$(xpath -q -e '/manifest/@android:versionName' tunnel/android/java/AndroidManifest.xml \
    | cut -d'"' -f2)

if [ "$TUNNEL_VERSION" != "$ANDROID_TUNNEL_VERSION" ]
then
    echo "Version name in Android manifest ($ANDROID_TUNNEL_VERSION) is not the same than the source version ($TUNNEL_VERSION)." 1>&2
fi
...
[ -z "$INSTALL_PREFIX" ] && INSTALL_PREFIX="$PWD/tunnel/android/java/"
...
cmake . \
    ...
    -DROLLER_PAINTING_ENABLED=* \
    -DTUNNEL_ENABLED=1 \
    -DTUNNEL_NO_MANPAGES=1 \
    -DBEAR_ENGINE_CORE_LINK_TYPE=STATIC \
    -DBUILD_PLATFORM="android" \
    -DCMAKE_INSTALL_PREFIX="$INSTALL_PREFIX" \
    -DTUNNEL_ANDROID_LAUNCHER_LINK_DIRECTORIES="$ANDROID_TOOLCHAIN_ROOT/arm-linux-androideabi/lib/" \
    -DTUNNEL_INSTALL_DATA_DIR="$DATA_DIR" \
    -DTUNNEL_INSTALL_ANDROID_LAUNCHER_DIR="libs/armeabi-v7a" \
    $CMAKE_ARGS
...
then
    ...
    ORIGINAL_APK=bin/Tunnel-release-unsigned.apk
    TARGET_APK=bin/Tunnel.apk
    ...
    ant debug
    TARGET_APK=bin/Tunnel-debug.apk
fi
