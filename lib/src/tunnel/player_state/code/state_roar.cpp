/*
  Copyright (C) 2012 Stuffomatic Ltd. <contact@stuff-o-matic.com>

  All rights reserved.
*/
/**
 * \file
 * \brief Implementation of the tunnel::state_roar class.
 * \author Sebastien Angibaud
 */
#include "tunnel/player_state/state_roar.hpp"

/*----------------------------------------------------------------------------*/
/**
 * \brief Constructor.
 */
tunnel::state_roar::state_roar( const player_proxy& player_instance )
  : state_player(player_instance)
{
} // state_roar::state_roar()


/*----------------------------------------------------------------------------*/
/**
 * \brief Return the name of the state.
 */
std::string tunnel::state_roar::get_name() const
{
  return "roar";
} // state_roar::get_name()

/*----------------------------------------------------------------------------*/
/**
 * \brief Move the player to the left.
 */
void tunnel::state_roar::do_move_left()
{
  // we can't move in this state
} // state_roar::do_move_left()

/*----------------------------------------------------------------------------*/
/**
 * \brief Move the player to the left.
 */
void tunnel::state_roar::do_move_right()
{
   // we can't move in this state
} // state_roar::do_move_right()

/*----------------------------------------------------------------------------*/
/**
 * \brief Do a jump.
 */
void tunnel::state_roar::do_jump()
{
   // we can't jump in this state
} // state_roar::do_jump()

/*----------------------------------------------------------------------------*/
/**
 * \brief Make the player doing slap.
 */
void tunnel::state_roar::do_slap()
{
   // we can't slap in this state
} // state_roar::do_slap()

/*----------------------------------------------------------------------------*/
/**
 * \brief Player throw a stone
 */
void tunnel::state_roar::do_start_throw()
{
 // we can't throw in this state
} // state_roar::start_throw

/*----------------------------------------------------------------------------*/
/**
 * \brief Player drop a power.
 */
void tunnel::state_roar::do_start_drop()
{
  // we can't drop in this state
} // state_roar::do_start_drop()

